...
from app.tasks.email_tasks import send_booking_cancellation
...
@router.delete(
    ...
    description="Cancel a booking. Only the booking owner can cancel. Sends cancellation email via Celery."
)
def cancel_booking(
    booking_id: int,
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        # Release the slot back to the trip (atomic update)
        ...
        try:
            ...
            logger.info(f"Booking cancellation email queued for {current_user.email}")
