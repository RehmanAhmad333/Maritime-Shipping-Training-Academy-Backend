...
@router.put(
    ...
    description="Update an existing trip. Requires admin role."
)
def update_trip(
    ...
    try:
        ...
        # Check for duplicate trip if location or date is changing
        if trip_data.location_name and trip_data.start_date:
            existing = db.query(Trip).filter(
                Trip.location_name == trip_data.location_name,
                Trip.start_date == trip_data.start_date,
                Trip.id != trip_id
        ...
        db.rollback()
        logger.error(f"Database error in update_trip: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
